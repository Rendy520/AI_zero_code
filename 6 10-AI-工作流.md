...
本节主要内容如下：EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
如果想快速实现这个需求，其实只⁠需要提供给 AI 图⁠片搜索工具就好，交给框架和 AI 来决定什么时候调用工具，自动执行。dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
💡 注意，跟之前已有的 3⁠ 种生成模式不同，⁠本节我们不是在开发一种新的生成模式，而是一套新的系统实现方式。qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=
...
因此，本节我们不改变原有的任何业务流程。只新增⁠一种实现，重点是让大家学习到工⁠作流的思想和实现方式。这样一来，本节内容能够作为可选项独立学习，不影响之前已完成的项目。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=

## 二、工作流方案选型

为了实现 AI 工作流，我们可以考虑市面上很多现成的平台，⁠像 Dify、Coze、阿里云百炼，通⁠过拖拽式的界面就能快速搭建工作流，上手简单、能够快速看到效果，非常适合快速验证想法或者让非技术人员参与构建流程。

![](https://pic.code-nav.cn/course_picture/1608440217629360130/QkvuabwkqsasZQF0.webp)cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
当然，更重要的是，LangGraph4j 作为一个专门为 Java 生态设计的工作流框架，**和 LangChain4j 兼容性很好**，能够无缝融入我们现有的技术栈，可以直接复用项目中已有的 Service、配置和工具类，不需要从 0 进行开发。同时，它还提供了丰富的高级特性，比如条件边、循环执行、并发处理等，让我们能够构建出智能化的工作流程。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=

从长远来看，选择编程框架而不是别人的低代码工作流平台，还有⁠一个重要优势：可维护性和扩展性。随着业⁠务的发展，我们的工作流可能会变得越来越复杂，编程框架能够更好地支持这种迭代，而不会像低代码平台那样遇到天花板。

## 三、LangGraph4j 入门

[LangGraph4j](https://langgraph4j.github.io/langgraph4j/) 是一个 Java 实现的开源 AI 工作流框架，它受到了 Python 版本 LangGraph 的启发，能够与 Langchain4j 和 Spring AI 无缝集成，而且这个框架还是 [开源](https://github.com/langgraph4j/langgraph4j) 的。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
不过好在鱼皮帮大家踩了一遍坑，我的建议是大家 **严格遵循本教程中的版本号**，收起你那好奇心，**不要用和鱼皮不一致的版本、也先不要自己探索**，把工作流的开发思路学会就足够了。后面框架肯定会持续更新的，你的能力上来后对着官方文档去了解新的变动就好。WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
它将复杂的 AI 工作流抽象为一个 **有向图** 的概念。每个 **节点** 代表一个具体的操作单元，比如调用 LLM 生成文本、搜索外部数据、处理用户输入等。节点之间通过 **边EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=** 来连接，多个节点之间通过 **状态** 来共享数据，形成了一个完整的处理流程。

和传统的有向无环图（DAG）不同，LangGra⁠ph4j 支持循环，比如一个智能⁠体可能需要根据结果回到之前的步骤进行重试，或者需要在某个条件满足之前持续循环执行某个逻辑。F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=

注意，使用图之前必须编译，编译过程不仅会进行基础的结构检查（比如检查是否有孤立的节点），还会定义运行时参数，创建一个不可变的、可运行的图 `CompiledGraph<S extends AgentState>`。

#### AgentState 状态

AgentState 是整个工作流的状态载体，它本质上是一个 `Map`，在不同节点之间传递。**每个节点都可以从这个状态中读取数据，并返回对状态的更新。**F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
2）Channel.Reducer 是状态更新的核心机制，它定义了如何将新值与现有值进行合并。比如，一个新值可能会覆盖旧值，或者可能会被添加到现有值的列表中。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
你可以根据具体的业务需求来定⁠义不同的 Redu⁠cer 策略，确保状态在整个工作流中正确地流转和更新。qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=
...
节点是图的构建块，**负责执行具体的操作**。UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
节点可以是同步的，也可以是异⁠步的，甚至可以多个⁠节点同时执行。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
这也是工作流的经典设计了，让⁠工作流的起始和结束⁠变得明确可控。dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=

#### Edges 边

边定义了节点之间的控制流，决⁠定了工作流的执行⁠路径。正如官方文档所说：“节点负责干活，边负责决定下一步”。

LangGraph4j 支持几种不同类型的边：F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
1）支持人类检查、中断和批准工作步骤。在某些场景下，人类必须能够在任何时间点查看图⁠的状态，并且图必须能够在人类对状态进行任何更新后恢复⁠执行。这种 “人在环路”（human-in-the-loop）的设计在许多实际应用中都是必需的，比如 Cursor 在使用文件删除工具时会找用户确认。UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
LangGraph4j 默认提供了基于内存的检查点器：EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
注意，使用以下版本的依赖：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
下面再分享一些 LangGr⁠aph4j 的高级⁠特性，先仅做了解即可，很多特性我们都会在本项目中实战。F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=

#### Streaming 异步和流式处理

首先，通过 CompletableFuture，La⁠ngGraph4j 允许非阻塞的异步⁠操作。也就是说，当某个节点在等待 LLM 响应时，整个应用不会被阻塞，可以继续处理其他任务。

根据 [流式处理文档](https://langgraph4j.github.io/langgraph4j/core/streaming/)，LangGraph4j 支持通过 Java 异步生成器来处理来自 LLM 和其他源的流式响应，可以实现 SSE 流式输出，提升用户体验。F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=

此外，流式处理不仅支持主流的输出，还能处理⁠子流，并且在所有流输出完成后⁠获取到最终结果。跟之前 LangChain4j 的 TokenStream 类似。

#### 可视化

LangGraph4j 通过 `graph.getGraph` 方法提供了多种内置的工作流可视化方式。可以选择使用 [PlantUML](https://plantuml.com/) 或者 [Mermaid](https://mermaid.js.org/) 文本绘图语法来生成工作流的结构图，帮助我们理解复杂工作流的结构。dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
但是目前并发处理存在一些限制⁠，比如不支持条件边⁠、并且并发执行的分支中只能有一个工作节点。qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=
...
有 3 种不同的使用方式，适合不同的场景。cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
2）编译添加：这种方式让子图和⁠父图保持一定的独立性⁠，只共享部分状态。适合需要一定独立性但又需要与主流程交互的场景。UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
有时我们需要在某个关键节点暂停执行，等待人工审核或确认，LangGraph4j 的 [断点功能](https://langgraph4j.github.io/langgraph4j/core/low_level/#breakpoints) 正是为这种场景设计的。1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
这个特性在实现 “人在环路” 的 AI 系统时⁠特别有用。比如在代码生成系统中⁠，可能希望在生成代码后暂停执行，让人工审核代码质量，确认无误后再继续后续的构建和部署流程。dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/6lDCRedX19b9Yde5.webp)cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=

Studio 支持和 Spri⁠ng Boot、Qu⁠arkus 和 Jetty 快速集成，本节后面我们也会实战。

#### Builder 构建工具

LangGraph4j 提供了一个 [可视化的工作流构建工具](https://github.com/langgraph4j/langgraph4j-builder)，可以通过拖拉拽的方式开发工作流，自动生成对应的 Java 代码。nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/pF0CmstWgmvKrRDz.webp "null")p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
注意，不要按照作者文档中的设置 `-e RUNNING_IN_DOCKER=true`，可能导致无法生成代码！EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/VdxTkAH62VRXDn88.webp)

确保服务成功启动：     ⁠         ⁠                  

![](https://pic.code-nav.cn/course_picture/1608440217629360130/J1lDQt3MCdwRkX4y.webp)F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=

第一次进入时，可以跟着网页的⁠提示学习用法，其实⁠就是画流程图：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/56LHHCerWP1vLEkv.webp)

构建器预置了 2 个模板：F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/JkZaz2BtIcbfaxca.webp)dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=

生成的代码可以给我们作为参考：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/o9bDeBzryK0RKXBf.webp)

不过需要注意的是，对于包含循环的复杂工作流，生成的代码可能⁠不够准确，而且会包含一些冗余内容，所以⁠我个人不太喜欢用这个工具（目前也没多少人用），还不如用 AI 来根据我们描述好的流程图来生成工作流代码。nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=

### 集成

LangGraph4j 可以快速和 Langchain4j / Spring AI 进行 [集成](https://langgraph4j.github.io/langgraph4j/integration/)。

比如通过 AgentExecuto⁠r，可以快速将 Cha⁠tModel 和 Tools 转换为 ReACT Agent 工作流：UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
其中我觉得自适应 RAG 系统⁠的示例是值得学习的，⁠它展示了如何根据用户描述智能选择搜索引擎和知识库来回答问题。nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
了解了 LangGraph4⁠j 的特性后，下面⁠我们直接在项目中实战。dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=

### 工作流开发步骤

工作流开发的核心是：节点 + 边 + 状态 + 其他特性

具体步骤：nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
根据我们的需求梳理出工作流程：1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
有了详细的工作流程描述后，可以利用 ⁠AI，根据 LangGr⁠aph4j 的官方文档和示例，快速生成基础的工作流结构代码，比如下列提示词：cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
执行工作流程序，可以看到输出⁠的工作流图结构（M⁠ermaid 语法）：qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/UvPEdGiqO3DAmJvO.webp)

以及每一步工作流的节点执行和状态信息：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/m2Vc2vnMWgaLtuuv.webp)qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=
...
用一个表格来梳理：UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=

1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=

|工作步骤|输入状态|输出状态p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=|
|---|---|---|
|图片收集|originalPrompt 原始提示词|images 图片资源列表每一个图片都应该是对象结构（图片类别、描述、地址）图片类别：content 内容图片URLsillustration 插画图片URLsarchitecture 架构图URLlogo Logo图片URLqo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=|
|提示词增强|originalPrompt 原始提示词 images 图片资源|enhancedPrompt 增强后的提示词，包含图片描述和引用p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=|
|智能路由⁠|originalPromptenhancedPrompt 增强后的提示词|generationT⁠ype 生成类型F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=|
|代码生成|enhancedPromptgenerationType 生成类型images|generatedCodeDir 生成的代码目录WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=|
|项目构建|generatedCodeDir 生成的代码目录|buildResultDir 构建成功的目录qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=|

#### 2、定义状态类

还记得么？官方提供的默认状态类是 **消息列表** 结构：EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
而且为了和 LangGraph4j 工作流图需要的 AgentState 兼容，我们可以将 WorkflowContext 对象作为一个 key / value 存放在 MessageState 中，需要使用时通过 `state.data().getKey` 获取即可。WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
图片类型枚举类：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
完善上一步得到的简单工作流：p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
智能路由节点：UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
执行并查看输出结果，应该和之⁠前一样，能够看到每⁠一个步骤的状态。WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=

接下来，需要依次开发真实的工作节点，工作量不小。

### 图片收集节点

图片收集节点的作用是根据用户提示词获取到网站所需的图片。UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/g2mo3PujszNumxAi.webp "null")F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
填写配置：1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
编写单元测试：p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/HAidc9to7uUrdSsi.webp "null")p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=

#### 3、架构图绘制工具

由于架构图是需要根据特定的描述来定制的，⁠不能直接上网搜索，因此我们⁠的思路是将 AI 调用工具时传入的 Mermaid 文本绘图代码转换成图片。

这里有几种方案：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/GtnX21HsDNzVdYo0.webp)qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=

3）Mermaid 文本生图 MCP：有一些 [开源的 MCP 服务](https://github.com/peng-shawn/mermaid-mcp-server)，优点是使用方便，缺点是不方便定制、而且图片会交给别人来存储。

推荐方案一，实现成本也不大。

首先安装必备的 mermaid-cli 工具：F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
编写单元测试：UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/QSXCtJSyKX3MsnNC.webp "null")EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=

#### 4、Logo 图片生成工具

由于 Logo 图片也是需要根据文字⁠定制生成的，因此需要用到⁠ AI 文生图模型，或者文生图 MCP 服务（比如 MiniMax）。

此处选择接入阿里云百炼平台的⁠ AI 大模型，会⁠比 MCP 服务更灵活。nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
2）从控制台 [获取 API Key](https://bailian.console.aliyun.com/?type=model&tab=model#/api-key)：cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
注意，虽然 API 支持一次性生成多张图片，但对我们来⁠说 1 张足够了，能省则省，毕竟程序⁠也判断不出来要用哪张 Logo。不过返回值还是使用列表，这时为了便于 AI 理解，生成失败就是列表为空。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=

这玩意挺贵的：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/1FTjFNXHhbi89Bwy.webp)

编写单元测试：EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/VAieXJqXgr0B866G.webp "null")EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
写一段 AI 收集图片的提示词，单独保存在资源文件 `image-collection-system-prompt.txt` 中：qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/7jyIseXHhU2rEh4N.webp "null")p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/LR785Zx0A6GXMBZw.webp)UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=

否则会报错：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/XQTafKjhF0Dm0RC6.webp "null")

结果，我测试阿里云百炼大模型⁠时发现，AI 的工⁠具调用意图被错误地尝试解析为结构化输出的返回值：UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
因此可以先复用原有的 Cha⁠tModel 配置⁠，无需新自定义 ChatModel。cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
2）创建 AI 服务创建工厂⁠，注入指定的 ch⁠atModel 和各种图片收集工具：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
执行效果如图，还挺不错的：p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/XmxbvY9LTYuWxa8F.webp)

#### 6、工作节点开发

工作节点需要调用 AI 服务⁠收集图片，并更新状⁠态的 imageListStr 字段。F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
在静态方法中获取指定 Bean：nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
拼接的提示词示例：EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
根据用户的原始提示词选择对应的网站生成方式，利用项目已有的 `AiCodeGenTypeRoutingService` 实现即可，逻辑参考 `createApp` 方法的实现。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
项目构建节点的代码如下：EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
最后，将原本的 WorkflowApp 改造为一⁠个新的工作流 CodeGenWo⁠rkflow，作为正式可用的类。不再是通过 main 方法运行，而是提供执行工作流的方法。nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
这里举个例子，比如让 AI ⁠做个技术博客，可以⁠通过 Debug 查看状态：F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/sEc8LQhr2AylznBx.webp)

查看生成的网站效果，已经得到了内容相关的图片：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/yGkRIqrXqBXAaDpy.webp)dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/OMRGwjtGaFyIQNtT.webp)qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=
...
💡 如果多次用到同一工作节⁠点的 key，建议⁠统一放到常量类中维护。UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
3）项目构建工作节点中移除 if-else 逻辑：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
用条件边的优势是：EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
新增代码质检 Agent 工作节点，⁠在生成代码后，检验代码是⁠否有异常、能否正常打包，如果有异常则返回到网站生成 Agent 重新生成。nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
原因如下：p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
2）根据质检结果的路由决策：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/DD33LQ1Q1drgEGb4.webp)F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
WorkflowContext 状态补充字段：1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
1）编写质量检查 AI 提示词 `code-quality-check-system-prompt.txt`，提示词重点突出 **检查语法错误**，并且强调结构化输出的 JSON 格式。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/2cxzIXNCchhGdxfL.webp)UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
2）新写一个路由函数，根据质检结果决定下一步，直接复用之前的 `routeBuildOrSkip` 方法：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
执行之前开发的工作流单元测试，可以看⁠到 AI 给出修改建议的⁠同时，isValid 仍然为 true，并没有触发多余的修改，效果不错：1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/Hq33c00jMgc5Nh1i.webp)

### 图片收集优化 - 并发和子图

目前通过工具调用获取图片需要⁠和 AI 进行多轮⁠交互，不仅耗费时间长、性能低、还会消耗大量 token。1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=

#### 优化方案

其实可以转变思路，先通过调用 AI 获取⁠要收集的图片类别和参数（输⁠出图片收集任务），利用结构化输出返回这些信息，然后并发调用对应的图片收集工具执行。

接下来，具体的并发实现方案有 3 种：qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=

1）工作节点内部实现并发（推荐）

在图片收集节点内部通过 Co⁠mpletable⁠Future 并发调用工具进行收集，并更新结果。

2）利用 LangGraph4j 的并发能力WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=

把每个图片收集工具定义成工作⁠节点，并发执行这些⁠工具，最后再统一汇总结果。

3）利用 LangGraph4j 的子图能力

把每个图片收集工具定义成子图⁠，并发执行这些工具⁠，最后再统一汇总结果。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=

其实 2 和 3 的思路类似，⁠但是建议只有并发分支⁠下有多个步骤时，才考虑使用子图能力，否则就有点为了封装而封装了。

#### 图片收集规划实现

1）编写 AI 收集图片的提示词 `image-collection-plan-system-prompt.txt`：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/TBXQu9tFDzGp061w.webp "null")WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=

查看生成的网站，显然有了图片⁠规划后，网站的生成⁠效果好了很多！

![](https://pic.code-nav.cn/course_picture/1608440217629360130/z5ompvXPTcG051fv.webp "null")

![](https://pic.code-nav.cn/course_picture/1608440217629360130/Pa201OUaIrRhZqg0.webp "null")UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
可以通过在图片收集节点内增加⁠ Spring 的⁠ StopWatch 来对比性能：nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/Bp1lMd5c07g5uzpb.webp)UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=

使用并发后，**性能提升了几倍，缩短了 70 多秒！**

![](https://pic.code-nav.cn/course_picture/1608440217629360130/8P835NKsjmLCWyer.webp)

💡 可以按需调整获取图片的最大数量，防止提示词过长。nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
工作流程变为：图片规划 => 并发收集 => 图片聚合qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=

如图：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/tCcpLBXnnmVDzqNP.webp)

WorkflowContext 状态新增字段：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
1）图片计划节点：分析用户需⁠求，生成图片收集计⁠划，为并发执行做准备qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=
...
4）架构图绘制节点：F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
上述代码中，我们只需要为同一⁠个工作节点创造连接⁠到多个不同图片收集工作节点的边，框架就会自动作为并发分支处理。WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
执行单元测试，但是查看日志发现竟然还是按顺序串行执行的？！1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
相关 issue 信息：p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
等并发分支都执行完后，才会进行下一步聚合图片：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/wCnUYG2nZNlX84j7.webp)

#### LangGraph4j 子图实现

其实在目前我们的场景里是用不到 [子图](https://langgraph4j.github.io/langgraph4j/core/parallel-branch/#use-compiled-sub-graph-as-parallel-node) 的，此处只是给大家作为示例。如果并发分支步骤超出 1 个，再考虑使用。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
执行单元测试，发现成功搜集到⁠了多类图片，子父图⁠状态完全共享：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/HSUsD93DGnN4fyEG.webp "null")

#### 三种并发方案对比

最后，再给大家提供上述 3 种并发方案的对比表：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=

WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=

|方面|方案1 (CompletableFuture)|方案2 (LangGraph4j 并发)1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=|方案3 (LangGraph4j 子图)|
|---|---|---|---|
|实现复杂度|中等|中等cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=|高|
|可维护性|中等|高nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=|最高|
|可重用性|低|中等UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=|⁠最高|
|独立测试|难|中等EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=|容易|
|状⁠态管理|手动|框架自动UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=|框架自动|
|可视化|单节点|多节点F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=|子图结构|
|团队协作|难|中等WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=|最佳|

鱼皮的建议是，如果你的系统本身就是基于工作⁠流进行设计实现的，采用方案 ⁠2 或方案 3。子图尤其适合需要模块化开发的大型项目、或者需要复用某些工作流片段的场景。

如果你只是单纯为了实现并发，方案 1 就足够了。1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
1）先引入依赖：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
我摸索了一番，发现访问后端根地址 `/api` 就能访问了，但默认会报错，因为请求接口的地址没有加 `/api` 前缀：cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/Q7O8d3Faoz7WDHcd.webp "null")

临时注释配置文件的 `/api` 路径，查看实际效果：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/E1j486jIi1BJ9fR8.webp "null")qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=

结果又是各种报错，我只能说目⁠前效果烂的一批，强⁠烈不推荐使用！还不如自己 Debug 调试。

### SSE 流式输出

之前我们已经通过 `workflow.stream` 获取到工作流的每一步内容，但如果想让某一个工作节点（比如网站生成）的内容也流式输出，就比较麻烦了。p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
下面给 2 种参考实现方案：p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
3）编写一个用于测试的前端网站：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/GOATxnlE823ozLtm.webp)nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
3）用 AI 生成工作流演示 Demo 网站：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/T7xp4ehuUmfu5jpq.webp)F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=

#### 扩展思路

在实际应用中，大家可以根据业务需求⁠灵活选择 SSE 输出⁠的内容，比如实时推送搜集到的图片信息、代码生成进度或者质量检查结果。

如果想要实现更细粒度的实时反馈，还可以考虑将 SseEmitter 传递⁠到具体的工作节点内部，让每个节点都能直接输出处理⁠状态。可以通过 ThreadLocal 在同一线程内传递 SseEmitter 对象，或者将其作为状态的一部分在节点间传递来实现。1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=

## 扩展思路 - 和现有业务整合

虽然开篇提到了，这是另一种实现模式；但如果一定要整合，其实也不麻烦，**但是我不建议大家实现，使用之前的模式就够了。**

以下仅为参考，实际要修改的内容可能会更多。p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
代码如下：p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
代码如下：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
而且工作流目前没有区分新建和⁠修改，如果是修改的⁠话，不需要重复搜图、增强提示词等步骤。UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=

## 总结

在这一节中，我们深入学习了 AI 工作流的概念和实⁠现。通过 LangGraph4j 框⁠架，我们不仅重构了一套新的代码生成逻辑，还为系统增加了智能图片收集功能，让生成的网站更加真实生动。

WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
