---
created: 2025-12-29T15:54:25 (UTC +08:00)
...
> ## Excerpt
> 本节重点 在前面的章节中，我们已经实现了对话历史功能，让 AI 能够基于历史上下文进行网站的迭代优化。但目前平台只支持生成原生网站，在实际使用中有一定局限性。 本节我们将挑战一个有点儿难。编程导航教程分享，做您编程学习路上的导航员。
...
本节我们将挑战一个有点儿难度⁠的功能 —— 生成⁠更复杂的前端工程化项目，提高本平台的实用性。dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/dYyU4hT4ENa4Oidm.webp)F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=

调研其他平台，美团 NoCo⁠de 平台支持 R⁠eact 工程项目：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/BLTeQqJi79uIX3WO.webp)

我们不妨就以 Vue 作为示例，让平台能够生成完整的 Vue3 + Vite 工程项目，并且跟其他两种生成模式一样，实现流式输出、网站浏览和部署。这样一来，我们的平台也能够生成更符合企业标准的项目。dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
这种方案延续之前的思路，直接⁠让 AI 在输出的⁠ Markdown 中包含代码块，然后通过解析的方式保存文件。WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/47Vy6q0qtGVsS7NN.webp)dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
也就是说，AI 的输出会把保存文件的路径和写入文件的⁠内容拆开。如果想在前端实时展示出这⁠些信息，就需要比较复杂的拼接逻辑，比如拼接好 relativePath 后，立刻展示出保存文件的路径。nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
想想就很复杂。。。而且不同工具的调用参数是不一样的！nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=

此外，由于我们有对话历史模块，需要⁠尽可能确保实时流式输出⁠的内容和保存到数据库中的对话历史一致，这就进一步增加了开发成本。

#### 方案 3 - Agent 模式

智能体（Agent）是指能够感知环境、进行推理、制定计划、做出决策并自主采取行动以实现特定目标的 AI 系统。它以大语言模型为核心，集成 **记忆、知识库和工具** 等能力为一体，构造了完整的决策能力、执行能力和记忆能力，就像一个有主观能动性的人类一样。p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
这样做的优点是，每一个步骤都非常清晰，在步骤内我可以⁠通过工具调用来实现文件保存，也可以⁠单独调用 AI 获取到生成的代码，然后再通过程序保存（就像我们之前一样），这样实现流式输出就很简单了。1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
💡 在 [编程导航的 AI 超级智能体项目](https://www.codefather.cn/course/1915010091721236482) 中，鱼皮带大家分析了 OpenManus 智能体的源码，并且复刻了一个 Java 智能体，感兴趣的同学可以学习。UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=

___

从美团 NoCode 的输出效果来看，很有⁠可能采用了 Agent 模式⁠，每一个步骤都可以做到流式输出，用户能即时看到生成的代码内容和 AI 的操作情况。

![](https://pic.code-nav.cn/course_picture/1608440217629360130/RaYuv7YJaCtBuUpt.webp "null")1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=

刷新页面后，你依然能看到上面⁠的内容，跟实时流式⁠输出时用户看到的效果一样。

这是怎么做到的呢？美团是怎么维护对话历史的呢？

通过分析网络请求，我们能获取⁠到后端返回给前端的⁠完整对话信息：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/tKBGxmFh6btl9Ely.webp)p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
实际运行中，即使是同一个提示⁠词，不同大模型的输⁠出效果也不同，大家可以根据自己的需要补充其他注意事项。F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/fSGGImIKFOao5jgT.webp)EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=

### 完整流程

生成完 Vue 工程代码后，是无法直接运行的，需要执行 `npm install` 命令安装依赖、执行 `npm run build` 打包构建，会得到一个打包后的 `dist` 目录，网站浏览和部署都应该是访问这个目录。

![](https://pic.code-nav.cn/course_picture/1608440217629360130/ykou20zszL3gfaMI.webp)dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
由于每个 appId 对应一个生成的网站，因此需要根据 appId 构造文件保存路径，可以利用 LangChain4j 工具调用提供的 **上下文传参** 能力。在 AI Service 对话方法中加上 memoryId 参数，然后就能在工具中使用 memoryId 了。UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
注意，工具必须要返回 **相对路径**，不能让 AI 把文件绝对路径返回给用户，会直接暴露服务器的存储路径！nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
记得先给 CodeGenTy⁠peEnum 补充⁠新的枚举类型：cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
💡 注意！这里最好做一些调⁠整，防止 AI 一⁠直无限循环调用工具，包括：nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
3）调整获取 AI Service 缓存的逻⁠辑。因为现在不同生成模式获取到的 A⁠I Service 不同，所以需要额外将 codeGenType 作为缓存 key 的构造条件：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
执行单元测试，会发现调用工具成功，文件成功保存。可以通过查看 Redi⁠s 中的对话记忆来分析 AI 的调用过程，发现⁠ AI 可以同时调用多个工具（如果用的是推理模型，这种现象会更明显）                                UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
虽然文件成功写入，但是目前要⁠等好久才会返回⁠结果。如何获取到工具调用的流式输出呢？UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/XlsizOup8Lqsb7mF.webp "null")nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
这样就实现了对源码的覆盖和增⁠强，给 Token⁠Stream 增加了两个回调函数。cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
工具调用完成时，触发 onT⁠oolExecut⁠ed 事件，可以获取到完整的工具调用参数：WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
但是比较难受的一点是，Tok⁠enStream ⁠只加了 3 个事件，并没有工具调用流式输出！dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/Pe45T0gH7BKthrZ8.webp)EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
我估计这是官方的一个小 Bug 吧，之后的版本应该会修复。1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
**因此，我们还是选择早期解决方案，通过覆盖依赖包源码的方式，给 TokenStream 加上流式输出。**1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=

大家直接从 [我的 GitHub 上获取这部分源码](https://github.com/liyupi/yu-ai-code-mother) 即可，相信官方会尽快给 TokenStream 加上对应的事件的。

-   📎dev.zip

[👆 资源获取文档](https://yuyuanweb.yuque.com/org-wiki-yuyuanweb-zvq1bg/webrf4/kquwk9bzruniczdg?singleDoc#)：密码 tpkbF+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/g9Wwyuwe23V5cYi7.webp)dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
这就要从需求出发了，考虑后端⁠对话记忆要保存什么⁠内容？前端需要看到什么内容？1vM0bC71h2RCfUkazMGszlP9tIpAZTvdkFrdc8O6/+w=
...
2）接下来，我们要统一封装消息，便于下游处理：nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
没错，我们可以在 AiCodeGeneratorFacade 门面类中编写一个 **适配方法**，将 TokenStream 转换为 Flux 对象，之后下游可以愉快地对 Flux 进行处理了，也便于返回给前端。WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
💡 由于之前 processCodeStream 方法也是⁠写在门面类里的，所以我才选择将 proc⁠essTokenStream 也写在同一个地方。其实也可以单独新建一个适配器类，会更符合适配器模式的基本写法。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
代码结构如下：nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
...
完整代码如下：cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
然后在前端和 AI 对话，查看运行效果，流式输出时：nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=

![](https://pic.code-nav.cn/course_picture/1608440217629360130/XnIxasSXtMdtfbAb.webp)

流式输出完成，刷新页面，查看加载的对话历史：

![](https://pic.code-nav.cn/course_picture/1608440217629360130/Znrzwh8bovjv2bKb.webp)EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=

效果符合预期，恭喜你，这块硬骨头终于啃下来了！

## 四、工程项目构建和浏览

代码生成后，需要安装依赖和打⁠包构建，才能浏览 ⁠Vue 项目。EVevk6+j/BC/ssKO5aUZF8Rusnkp1o/HghJkPkRChGQ=
...
2）然后分别编写执行安装依赖和执行打包构建命令的方法：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
编写操作系统检测方法：cZq9/nGwbRXl38gxWuOGAJfnulyo9Sx2vt9GhAJuMwQ=
...
3）编写构建项目的方法，组合⁠执行上述命令，并且⁠校验是否构建成功。qo3wYiJtEuojZDmpW2HRDEwGWUH1DzVFQ47Zs4YmYuk=
...
4）由于打包构建是个耗时操作，⁠为了不阻塞主流程，可⁠以使用 Java 21 的虚拟线程特性，在单独的线程中执行构建。WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
5）JSON 消息流式处理器 Js⁠onMessageSt⁠reamHandler 中，当流式输出完成后，执行 Vue 项目的构建：F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
1）补充枚举类型：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/oSyvLEA65rnEboPW.webp "null")WBpr9WI72APgOj+DCCZKObs+ofP0VhBHSvqSELGriv0=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/c5ooPKAMbejwjVBm.webp "null")F+kzRHG1248BgN91hBPqtNxbML3nLzvb0nw4jU7NTyI=
...
💡 注意！如果你发现 AI⁠ 一直无限循环调用⁠工具，可以尝试下面的方法：dWCTYpHEFA/xV8BrvOpKRqkO1d88b9Ip8VkvBeLuZho=
...
![](https://pic.code-nav.cn/course_picture/1608440217629360130/bNP6DB665tUc4bSa.webp)p+CfRPZxNEuU7hG8dzfPcN4R10eC9eypGHmTaRf/x7c=
...
比如追加这么一段提示词：UzapLzatIvJJsWWs1pRz7879eXluI/+jA6d4qBCHuKE=
...
但是要注意：nShO7+0zZStNyiJdnnoFDwT4JyyptzNsRjyXLsolAAU=
